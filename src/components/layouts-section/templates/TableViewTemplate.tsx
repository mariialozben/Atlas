...
export default function TableViewTemplate() {
  return (
    <div data-el="template-table-view" className="overflow-y-auto h-[500px] rounded-lg border border-white/10" style={{ backgroundColor: 'var(--bg-page)' }}>
        ...
        <TableBody>
          {TABLE_ROWS.map((row) => (
            <TableRow key={row.name}>
              ...
              <TableCell className="text-xs" style={{ color: 'var(--text-muted)' }}>{row.type}</TableCell>
              <TableCell className="font-mono text-xs" style={{ color: 'var(--text-muted)' }}>{row.value}</TableCell>
              <TableCell>
